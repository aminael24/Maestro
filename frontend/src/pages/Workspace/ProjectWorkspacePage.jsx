...
const ProjectWorkspacePage = () => {
  ...
  useEffect(() => {
    ...
    // Liaison réelle : Charger les fichiers du projet depuis le WorkspaceService
    const loadProjectData = async () => {
      try {
        ...
        const results = await Promise.all(
          mapping.map(m => 
            api.get(`/api/projects/${projectId}/files/content`, { params: { path: m.path } })
        ...
        const loadedFiles = {};
        results.forEach(r => { if (r?.content) loadedFiles[r.key] = r.content; });

        // On met à jour le store avec les vrais fichiers du disque
        if (Object.keys(loadedFiles).length > 0) {
          useAiStore.setState({ 
            files: { ...useAiStore.getState().files, ...loadedFiles },
            activeFile: Object.keys(loadedFiles).includes("frontend") ? "frontend" : Object.keys(loadedFiles)[0]
          });
        }
      } catch (err) {
  ...
  return (
    <div style={{
      ...
      <main style={{
        ...
        background: '#111d27', overflow: 'hidden',
      }}>
        <CodeEditor />
      </main>
      ...
      <section style={{
        ...
        background: '#0d1a24', overflow: 'hidden',
      }}>
