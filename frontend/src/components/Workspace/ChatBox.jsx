...
// ✅ MODIFIÉ : ajout de projectType en prop
const ChatBox = ({ projectId, projectType }) => {
  const [message, setMessage] = useState("");
  const [chatHistory, setChatHistory] = useState(() => {
    const saved = projectId
      ? localStorage.getItem(`chat_history_${projectId}`)
      : null;
    return saved
      ? JSON.parse(saved)
      : [
          {
            role: "ai",
            text: "Bonjour ! Quel projet CRUD souhaitez-vous créer aujourd'hui ?",
          },
        ];
  });

  const { generateNewProject, askModification, isLoading, error } =
    useAiAssistant();
  const {
    explanation,
    files,
  ...
  const handleSend = async () => {
    ...
    if (!hasGeneratedFiles && !snap.code) {
      // ✅ MODIFIÉ : on passe le projectType à generateNewProject
      await generateNewProject(
        userMsg,
        projectType?.toLowerCase() || "fullstack",
      );
        ...
        Object.entries(mapping).forEach(([key, path]) => {
          if (generatedFiles[key]) {
            api
              ...
              .catch((err) =>
                console.error(`Erreur de synchro initiale pour ${path}:`, err),
              );
          }
        });
      }
      ...
      const response = await askModification(
        userMsg,
        files,
        snap.code,
        snap.file,
    ...
    >
      ...
      >
        <div
          style={{
            ...
            height: "32px",
            borderRadius: "8px",
            background: "linear-gradient(135deg, #1a3a5c, #0d5ea3)",
            display: "flex",
            alignItems: "center",
        ...
        <div>
          <div
            style={{ color: "#e8f4fd", fontSize: "13px", fontWeight: "600" }}
          >
            Assistant IA Maestro
          </div>
          <div style={{ color: "rgba(255,255,255,0.3)", fontSize: "11px" }}>
            Génération & modification de code
          </div>
        </div>
        <div
          style={{
      ...
      {selectedCode && (
        ...
        >
          ...
          >
            {selectedCode.slice(0, 150)}
            {selectedCode.length > 150 ? "…" : ""}
          </pre>
        </div>
      )}
      ...
      <div style={{ padding: "16px" }}>
        ...
        >
          <textarea
            ...
            placeholder={
              selectedCode
                ? `Que faire avec ce code ? (ex: ajoute la validation)`
                : `ex: Crée une gestion de bibliothèque...`
            }
            rows={2}
            style={{
              flex: 1,
          ...
          <button
            ...
            style={{
              ...
              justifyContent: "center",
              color:
                isLoading || !message.trim() ? "rgba(255,255,255,0.3)" : "#fff",
              flexShrink: 0,
              transition: "all 0.2s",
            }}
          >
