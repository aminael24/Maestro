...
public class ProjectService(WorkspaceDbContext db, ILogger<ProjectService> logger) : IProjectService
{
    ...
    public async Task InitializeProjectAsync(int projectId, ProjectType type)
    {
        ...
        var rootPath = GetProjectRootPath(project); // ← /app/projects/{keycloakId}/{projectId}/
        ...
        try
        {
            ...
            switch (type)
            {
                ...
                // Cas "Empty" : juste un README, l'IA fera le reste
                default:
                    break;
            }
            ...
            await db.SaveChangesAsync();
            logger.LogInformation("[Init] Projet {ProjectId} → status 'active'. Chemin : {RootPath}", projectId, rootPath);
        }
        catch (Exception ex)
        {
    ...
    private async Task CreateFrontendTemplate(string path)
    {
        ...
        await File.WriteAllTextAsync(
            Path.Combine(path, "src", "App.jsx"),
            "export default function App() { return <h1>Maestro</h1>; }"
        );
    }
